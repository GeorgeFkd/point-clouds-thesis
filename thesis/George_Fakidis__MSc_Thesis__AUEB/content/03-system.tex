...
We use an Intel Realsense camera model D435I as seen in \ref{fig:related:2}.Its technical specs are the following:
\begin{itemize}
	\item \emph{Resolution}: 1280x720
	\item \emph{Depth Field of View}: 87 degrees wide horizontally, 58 degrees vertically
	\item \emph{FPS}: 30
	\item \emph{Operating range}: Min. 0.3m, Max 3m
\end{itemize}
For a more complete view of the technical specifications one can consult \href{https://www.intel.com/content/www/us/en/products/sku/190004/intel-realsense-depth-camera-d435i/specifications.html}{Intel's official specification}.

The setting is at an office, an indoor place, with either not harsh natural light or
normal overhead office lights.
A simple rendering of a point cloud can be seen in \ref{fig:system:1}.
What we want to achieve is to cover people and faces that appear in the point cloud frame of the camera using a simple blurring effect.
There are more advanced techniques for anonymization as previously discussed, such as the sophisticated face swapping demonstrated in \cite{Zhao_Zhang_Demiris_2024}.

\begin{figure}[hb]
	\includegraphics[width=0.45\textwidth]{figs/Pointcloud-Face-Without-Blur.pdf}
...
\begin{figure}[h]
	\includegraphics[width=0.75\textwidth]{figs/Face-Blurred-With-2D-Detection-And-Blur.pdf}
...
provided with pretrained weights from a library we used(open3d-ml, discussed in section \ref{sec:system:sec2}). The results can be seen in figure \ref{fig:system:4}.
\begin{figure}[htb]
...
The failure most likely stems from improper configuration for our scenario and the specific camera specifications.
Another likely root cause could be that such models are trained on datasets unlike our use case, both in setting and
in scale(LiDAR scans often cover tens of meters ahead while our camera can only reach 3 meters).
...
For 3D object detection the PointPillars model was used as the research indicated it is reasonably fast and within this library it is implemented and pretrained on the KITTI dataset in both PyTorch and Tensorflow implementations.
